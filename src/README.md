...
### Standalone Version
In the root of this project, you will find `electricity_calc_standalone.html`. This file is automatically generated by the build script — it inlines all HTML, CSS, and JS from this folder into a single self-contained file. It is the recommended version for daily use while this folder remains the primary workspace for development.

To regenerate it locally:

```bash
node build.js   # from the repository root
```

The GitHub Actions APK build also runs this automatically, so the standalone file stays up to date with every release.
