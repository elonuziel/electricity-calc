 ...
 * @param {Array} cloudBills - Bills from cloud backup
 * @param {Object} cloudSettings - Settings from cloud backup
 * @param {Function} onConfirm - Callback function to run on confirm (no string injection)
 */
function showCloudRestoreModal(cloudBills, cloudSettings, onConfirm) {
    ...
    modal.innerHTML = `
        <div class="bg-white rounded-lg p-6 max-w-md">
            ...
            <div class="flex gap-3">
                <button id="cloudRestoreCancelBtn" class="flex-1 bg-gray-300 text-gray-800 px-4 py-2 rounded hover:bg-gray-400">
                    ביטול
                </button>
                <button id="cloudRestoreConfirmBtn" class="flex-1 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                    החלף הכל
                </button>
            </div>
        </div>
    ...
    modal.querySelector('#cloudRestoreCancelBtn').addEventListener('click', () => modal.remove());
    modal.querySelector('#cloudRestoreConfirmBtn').addEventListener('click', () => {
        onConfirm();
        modal.remove();
    });
}
...
async function saveToCloud() {
    // Check if online
    if (!isOnline()) {
        showCloudStatus('לא ניתן לשמור נתונים כשאתה לא מחובר לאינטרנט.', true);
        return;
    }
    ...
    try {
        ...
        if (backupId) {
            ...
            url = `${CONFIG.CLOUD.CORS_PROXY}${encodeURIComponent(CONFIG.CLOUD.API_URL + '/' + backupId)}`;
            method = 'PUT';
            body = JSON.stringify({
                editKey: accessKey,
                data: payload
            ...
            // Create new backup
            url = `${CONFIG.CLOUD.CORS_PROXY}${encodeURIComponent(CONFIG.CLOUD.API_URL + '/save')}`;
            method = 'POST';
            body = payload;
        }

        const res = await fetch(url, { method, headers, body, signal: AbortSignal.timeout(CONFIG.CLOUD.FETCH_TIMEOUT) });
